...
def predict(image_name):
    ...
    # Running model on batch of images
    preds = model.predict(x_batch)
    
    # We can get and print the predicted label 
    # with the highest probability
    pred = resnet50.decode_predictions(preds, top=1)[0]
   

    class_name = pred[0][1]
    pred_probability = round(pred[0][2],4)
...
def classify_process():
    ...
    while True:
        ...
        # Decoding the JSON data for the given job
        q = json.loads(q.decode("utf-8"))

        # Running the model
        prediction, score = predict(q["image"])
        ...
        # Getting and keep the original job ID
        job_id = q["id"]
        
        # updating the key value store, so the client can get the results.
        db.set(job_id, json.dumps(model_output))
